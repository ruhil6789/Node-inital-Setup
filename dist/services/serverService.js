...
export const sendSuccessRes = (res, message, data, statusCode = StatusCodes.OK) => res.status(statusCode).json({
    status: "success",
    statusCode,
    message,
    data,
...
export const runTheMainServer = (app) => {
    const server = app.listen(PORT, () => {
        logger.info(`Server is running @ http://127.0.0.1:${PORT}`);
    });
    const handleExit = () => {
        console.warn(`Process ${process.pid} is exiting. Closing server gracefully...`);
        server.close(() => {
...
export const runTheServer = async (app, io, runOnThreads = true, hasDbConnection = false) => {
    try {
        if (runOnThreads) {
...
export const runOnServerOnThreads = (app, io) => {
